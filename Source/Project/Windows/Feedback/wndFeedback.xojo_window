	...
	#tag Method, Flags = &h21
		Private Sub SendMail()
		  ...
		  if popFeedbackType.ListIndex = 0 then
		    ...
		    message = message + Str( cvsRating.Rating ) + " Stars" + "<br/>"
		    message = message + "<br/>"
		  end if
		  
		  if trim( edtEmail.Text ) <> ""  then
		    message = message + "<h2>_______ eMail Address _______</h2>" + "<br/>"
		    message = message + trim( edtEmail.Text ) + "<br/>"
		    message = message + "<br/>"
		  end if
		  ...
		  self.Close
		    ...
		    MailSocket.Username = "vidal.vanbergen@gmail.com" 'AppSettings.kgmail
		    MailSocket.Password = "Back_To_The_Future_86" 'DecodeHex( AppSettings.kgmailEncPassword )
...
#tag Events edtSystemSettings
	#tag Event
		Sub Open()
		  ...
		  settings = settings + "<b>Automatic Updates:</b> " + Str( AppSettings.Prefs.Value( "KUCheckForUpdatesAtStartUp", False ) ) + "<br/>"
		  settings = settings + "<b>Show Notifications:</b> " + Str( AppSettings.Prefs.Value( "ShowNotifications", True ) ) + "<br/>"
		  settings = settings + "<b>PreFix Link Type:</b> " + Str( AppSettings.Prefs.Value( "PrefixLinkType", False ) ) + "<br/>"
		  settings = settings + "<b>PreFix Link Name:</b> " + Str( AppSettings.Prefs.Value( "PrefixFeedName", False ) ) + "<br/>"
		  settings = settings + "<b>Organize by Feed:</b> " + Str( AppSettings.Prefs.Value( "OrganizeByFeed", False ) ) + "<br/>"
		  settings = settings + "<b>Prefer Magnets:</b> " + Str( AppSettings.Prefs.Value( "PreferMagnets", False ) ) + "<br/>"
		  settings = settings + "<b>Check Interval:</b> " + Str( AppSettings.Prefs.Value( "CheckInterval", "30" ) ) + "<br/>"
		  settings = settings + "<b>Recent links count:</b> " + Str( AppSettings.Prefs.Value( "RecentCount", "30" ) ) + "<br/>"
		  settings = settings + "<b>DownloadsFolder:</b> " + Str( AppSettings.Prefs.Value("DownloadsFolder", "") ) + "<br/>"
		  
		  dim Feeds as Dictionary = HydRaSSCore.RSSFeeds
		  if feeds <> Nil then
		    settings = settings + "Feeds:<br/>"
		    for each key as string in Feeds.Keys
		      dim entry as Dictionary = Feeds.Value(key)
		      settings = settings + "FeedName: " + entry.Lookup("name","") + "<br/>"
		      settings = settings + "FeedTag: " + entry.Lookup("tag","") + "<br/>"
		      settings = settings + "FeedURL: " + entry.Lookup("url","") + "<br/><br/>"
		    next
		  end if
